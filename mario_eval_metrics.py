...
def compute_psnr(original: np.ndarray, generated: np.ndarray) -> float:
    """Peak Signal-to-Noise Ratio (↑ higher is better) - Auto-detects range"""
    # 1. Check for data range mismatch
    orig_max = original.max()
    gen_max = generated.max()
    
    # Heuristic: If max > 1.1, assume 0-255. Else assume 0-1.
    is_0_255 = (orig_max > 1.1 or gen_max > 1.1)
    max_val = 255.0 if is_0_255 else 1.0
...
class MarioEvaluator:
    ...
    def _preprocess_for_lpips(self, img_np):
        """Helper: Robustly normalize numpy image to [-1, 1] torch tensor"""
        # Convert to float32
        ...
        # 2. Normalize to [-1, 1] (LPIPS requirement)
        img = img * 2.0 - 1.0
        
        # 3. To Tensor (H, W, C) -> (1, C, H, W)
    ...
    def evaluate_visual_fidelity(self, original_frames: np.ndarray, 
        ...
        for orig, gen in zip(original_frames, generated_frames):
            ...
            t_orig = self._preprocess_for_lpips(orig)
            t_gen = self._preprocess_for_lpips(gen)
            score = self.lpips_fn(t_orig, t_gen)
