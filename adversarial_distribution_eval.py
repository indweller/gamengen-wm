...
def compute_l0_perturbation(original: np.ndarray, adversarial: np.ndarray, 
    ...
    """L0 norm - percentage of pixels changed. Auto-thresholds based on data type."""
    diff = np.abs(original.astype(float) - adversarial.astype(float))
    
    if threshold is None:
        if original.max() <= 1.0:
            threshold = 1.0 / 255.0  # Equivalent to 1 pixel value difference
        else:
            threshold = 1.0
...
def compute_kl_divergence(p: np.ndarray, q: np.ndarray, bins: int = 256) -> float:
    """KL Divergence with auto-ranging for [0,1] or [0,255]"""
    p_flat = p.flatten()
    q_flat = q.flatten()
    
    max_val = max(p_flat.max(), q_flat.max())
    data_range = (0, 1) if max_val <= 1.0 else (0, 255)
    
    # Calculate histograms with dynamic range
    p_hist, _ = np.histogram(p_flat, bins=bins, range=data_range, density=True)
    q_hist, _ = np.histogram(q_flat, bins=bins, range=data_range, density=True)
...
def compute_frechet_distance(original_feats: np.ndarray, adversarial_feats: np.ndarray) -> float:
    """
    Computes Fréchet Distance (Renamed from FID to avoid confusion).
    Includes numerical stability fixes.
    """
    mu1 = np.mean(original_feats, axis=0)
    sigma1 = np.cov(original_feats, rowvar=False)
    
    mu2 = np.mean(adversarial_feats, axis=0)
    sigma2 = np.cov(adversarial_feats, rowvar=False)
    
    # FIX: Handle 1D case (single feature)
    if sigma1.ndim == 0:
        sigma1 = np.atleast_2d(sigma1)
        sigma2 = np.atleast_2d(sigma2)

    # FIX: Add epsilon for numerical stability (regularization)
    eps = 1e-6 * np.eye(sigma1.shape[0])
    sigma1 += eps
    sigma2 += eps
